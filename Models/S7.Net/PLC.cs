    ...
    public class Plc : IDisposable
    {
        ...
        public bool IsAvailable
        {
            get
            {
                ...
                {
                    bool Returned = Connect(socket) == ErrorCode.NoError;
                    //if (Returned)
                    //    _mSocket = socket;
                    return Returned;
                }
        ...
        public Plc(CpuType cpu, string ip, Int16 rack, Int16 slot)
        {
            if (!Enum.IsDefined(typeof(CpuType), cpu))
                throw new InvalidEnumArgumentException(cpu.ToString(), (int) cpu, typeof(CpuType));

            if (string.IsNullOrEmpty(ip))
                throw new ArgumentException("IP address must valid.", ip);//nameof(ip));
        ...
        //Same as Read but with out Byte Array
        public object Read(DataType dataType, int db, int startByteAdr, VarType varType, int varCount, out byte[] arrByte, byte bitAdr = 0)
        {
            arrByte = new byte[4];
            int cntBytes = VarTypeToByteLength(varType, varCount);
            byte[] bytes = ReadBytes(dataType, db, startByteAdr, cntBytes);
        ...
        /// <summary>
        /// the same as Read Function but force the vartype to obtain 
        /// </summary>
        /// <param name="variable"></param>
        /// <param name="objExternalVAlrType"></param>
        /// <returns></returns>
        public object Read(string variable,VarType objExternalVAlrType)
        {
            DataType mDataType;
            int mDB;
            int mByte;
                    ...
                    case "DB":
                        ...
                        switch (dbType)
                        {
                            ...
                            case "DBW":
                                ...
                                return objI;
                            case "DBD":
                                if (objExternalVAlrType == VarType.Real)
                                {
        ...
        //same read But With out bytearray
        public object Read(string variable,out byte[] arrByte)
        {
            arrByte = new byte[4];
            DataType mDataType;
            int mDB;
                    ...
                    case "DB":
                        ...
                        switch (dbType)
                        {
                            ...
                            case "DBD":
                                ...
                                UInt32 objU = (UInt32)Read(DataType.DataBlock, mDB, dbIndex, VarType.DWord, 1,out arrByte);
                                double dblTemp = S7.Net.Types.Double.FromDWord(objU);
                                //decimal objU = (decimal)Read(DataType.DataBlock, mDB, dbIndex, VarType.Real, 1);
                                return objU;
                            case "DBX":
        ...
        {
            if (value < 0 || value > 1)
                throw new ArgumentException("Value must be 0 or 1",value.ToString());
            ...
            if (bitAdr != -1)
            {
                ...
                else if (value is int)
                {
                    ...
                    if (intValue < 0 || intValue > 7)
                        throw new ArgumentOutOfRangeException(string.Format("Addressing Error: You can only reference bitwise locations 0-7. Address {0} is invalid", bitAdr), bitAdr.ToString());

                    bitValue = intValue == 1;
                }
                else
                {
                    throw new ArgumentException("Value must be a bool or an int to write a bit", value.ToString());
                }
