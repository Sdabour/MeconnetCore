...
function FillServiceGroup() {


    var vrServiceUrl = "api/MeasurementGroupAPI";
    var vrIsDateRange = 0;

    var vrStart = new Date().toISOString().substring(0, 10);
    var vrEnd = new Date().toISOString().substring(0, 10);
    if (document.getElementById("chkGroupDateRange") != null) { vrIsDateRange = document.getElementById("chkGroupDateRange").checked ? 1 : 0; }
    if (vrIsDateRange == 1) {
    ...
    function successFunc(data, status) {
        ...
        var vrMeterGroup = new MeterGroup();
        var vrStrArr;
        var vrStrCard = "";
        for (var vrIndex = 0; vrIndex < data.length; vrIndex++) {

            strDV += vrMeterGroup.GetRow(data[vrIndex]);
        ...
        setTimeout(FillServiceGroup, 5000);
...
function FillMeterMeasure() {
    ...
    var vrServiceUrl = "../api/MeterMeasureAPI";
